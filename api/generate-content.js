...
【ルール】
- テーマがある時は、そのテーマを中心に具体的に書く。一般的な店紹介で終わらせない。書き出し・構成・語り口を上の「切り口」に沿って毎回変え、前回と同じ言い回しの繰り返しを避ける。
- テーマ欄以外で、店舗情報に無い事実（架空のメニュー・価格・イベント等）は創作しない。
- 指示の言葉自体を店名やお店のコンセプトとして誤用しない（例：テーマが「MEO」でも「MEOという店/概念」の話にはせず、この店が実際に提供する内容に引きつける）。
- 「店名:」「業種:」等のラベルは投稿文に書かない。自然な文章に溶け込ませる。地域名・サービス名を自然に含める。
- 150〜250文字程度・読みやすい改行・絵文字1〜2個まで・来店/問い合わせを促す一言を1つ・効果や順位の断定・誇大表現はしない。
- 投稿文のみ出力（前置き・説明は書かない）。`;
  }
